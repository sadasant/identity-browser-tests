...
test("Authenticates", async ({ page }) => {
  ...
  await page.evaluate(
    ({ clientId, protocol, host, port, scope }) => {
      ...
      const { newRedirectCredential } = window as any;
      ...
      const credential = newRedirectCredential({
        clientId,
  ...
  await page.evaluate(
    async ({ clientId, protocol, host, port, scope }) => {
      ...
      while (!(window as any).newRedirectCredential) {
        await delay(100);
      }
      const { newRedirectCredential, credentialWrapper } = window as any;
      ...
      const authenticationRecord = window.localStorage.authenticationRecord ? JSON.parse(window.localHost.authenticationRecord) : undefined;
